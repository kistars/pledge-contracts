...
contract MockPledgePool is ReentrancyGuard, Ownable, SafeTransfer {
    ...
    constructor(address _oracle, address _swapRouter, address payable _feeAddress) Ownable(msg.sender) {
    ...
    function setPoolState(uint256 _pid, uint256 _state) external onlyOwner {
     ...
     */
    function updatePoolBaseInfo(uint256 _pid, uint64 _interestRate, uint256 _maxSupply, uint256 _autoLiquidateThreshold)
        public
        onlyOwner
        timeBefore(_pid)
    {
     ...
     */
    function depositBorrow(uint256 _pid, uint256 _stakeAmount, uint256 _deadLine)
        external
        payable
        nonReentrant
        notPause
     ...
     * @param _deadLine is final deadline
     */
    function withdrawBorrow(uint256 _pid, uint256 _jpAmount, uint256 _deadLine)
        external
        nonReentrant
        notPause
        timeDeadline(_deadLine)
