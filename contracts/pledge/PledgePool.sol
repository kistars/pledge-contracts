...
contract PledgePool is ReentrancyGuard, SafeTransfer, multiSignatureClient {
    ...
    struct PoolBaseInfo {
        ...
        uint256 lendSupply; // 当前池子总存入的出借量
        uint256 borrowSupply; // 当前池子总存入的借款量
        ...
        stateMatch(_pid)
    {
        ...
        //
        require(_stakeAmount <= (pool.maxSupply).sub(pool.lendSupply), "depositLend: exceeds limit");
        uint256 amount = getPayableAmount(pool.lendToken, _stakeAmount);
        require(amount > minAmount, "depositLend: less than limit");
        // 保存出借用户信息
        ...
        stateFinishLiquidation(_pid)
    {
        ...
        require(_spAmount > 0, "withdrawLend: amount equals 0");
        // 销毁 sp_token
        ...
        if (pool.state == PoolState.FINISH) {
            require(block.timestamp > pool.endTime, "withdrawLend: less end time");
        ...
        if (pool.state == PoolState.LIQUIDATION) {
            require(block.timestamp > pool.settleTime, "withdrawLend: less settle time");
            // 赎回金额
    ...
    function claimBorrow(uint256 _pid) external nonReentrant notPause timeAfter(_pid) stateNotMatchUndone(_pid) {
        ...
        // 限制
        require(borrowInfo.stakeAmount > 0, "claimBorrow: stake amount less than 0");
        require(!borrowInfo.hasNoClaim, "claimBorrow: claim again");
    ...
    function settle(uint256 _pid) public validCall {
        ...
        // 池子的状态必须是匹配状态
        require(pool.state == PoolState.MATCH, "settle: state shoule be MATCH");

        if (pool.lendSupply > 0 && pool.borrowSupply > 0) {
            // 获取标的物价格
            uint256[2] memory prices = getUnderlyingPriceView(_pid);
            // 总保证金价值 = 保证金数量 * 保证金价格 (amount * token1Price / token2Price)
            ...
            if (pool.lendSupply > actualValue) {
                ...
                data.settleAmountBorrow =
                    pool.lendSupply.mul(pool.martgageRate).div(prices[1].mul(baseDecimal).div(prices[0]));
            }
            // 更新池子状态
    ...
    function finish(uint256 _pid) public validCall {
        ...
        // amountIn: 卖掉资产后，能得到多少钱, lendAmount: 本金加利息
        ...
        data.finishAmountBorrow = remainBorrowAmount; // 池子里最终剩下的质押资产
    ...
    function liquidate(uint256 _pid) public validCall {
        ...
        require(pool.state == PoolState.EXECUTION, "liquidate: must be execution"); // 需要池子的状态是执行状态
    ...
    function _swap(address _swapRouter, address token0, address token1, uint256 amount0) internal returns (uint256) {
        if (token0 != address(0)) {
            _safeApprove(token0, address(_swapRouter), uint256(0));
        }
        if (token1 != address(0)) {
            _safeApprove(token1, address(_swapRouter), uint256(0));
        }
        IUniswapV2Router02 IUniswap = IUniswapV2Router02(_swapRouter);
